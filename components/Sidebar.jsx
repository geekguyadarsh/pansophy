...
export default function Sidebar() {
  ...
  return (
    <>
      <button onClick={toggleSidebar} className="absolute top-3 left-10">
        <Image className="" src={Toggle} alt="toggle" />
      </button>
      <div
        className={`${
          isCollapsed ? "hidden md:w-16 w-16" : "w-2/3 md:w-1/5 z-50"
        } h-screen bg-[#0C0D0F] fixed transition-all duration-300`}
      >
      ...
      <div
        className={`${
          isCollapsed ? "md:w-16 w-16 hidden" : "w-2/3 md:w-1/4"
        } h-screen`}
      ></div>
    </>
  );
